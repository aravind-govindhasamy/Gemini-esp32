...
esp_err_t settings_read_parameter_from_nvs(void) {
  ...
  /* --- CREDENTIALS FROM NVS OR DASHBOARD --- */
  /* Default credentials for testing - can be overwritten via dashboard */
  strncpy(g_sys_param.ssid, "BUDE-ESP-32", SSID_SIZE);
  strncpy(g_sys_param.password, "Pass###1234", PASSWORD_SIZE);
  strncpy(g_sys_param.gemini_key, "AIzaSyDbcsKOqM7Tley4WsVMn-najFIFcetOOtg",
          KEY_SIZE);
  ...
  if (nvs_get_str(my_handle, "password", g_sys_param.password, &len) !=
      ESP_OK) {
  ...
  if (nvs_get_str(my_handle, "Gemini_key", g_sys_param.gemini_key, &len) !=
      ESP_OK) {
  ...
  if (nvs_get_str(my_handle, "wit_token", g_sys_param.wit_token, &len) !=
      ESP_OK) {
  ...
  if (nvs_get_str(my_handle, "user_name", g_sys_param.user_name, &len) !=
      ESP_OK) {
...
sys_param_t *settings_get_parameter(void) { return &g_sys_param; }
