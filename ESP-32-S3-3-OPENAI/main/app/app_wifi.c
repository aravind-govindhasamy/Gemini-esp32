...
#define EXAMPLE_ESP_MAXIMUM_RETRY                                              \
  20 // Increased from 5 to 20 for better persistence
...
esp_err_t send_network_event(net_event_t event) {
  ...
  ESP_RETURN_ON_FALSE(pdPASS == ret_val, ESP_ERR_INVALID_STATE, TAG,
                      "The last event has not been processed yet");
  return ESP_OK;
}
...
static void wifi_scan(void) {
  ...
  if (ap_count && (ESP_OK == ret)) {
    scan_info_result.ap_count =
        (ap_count < DEFAULT_SCAN_LIST_SIZE) ? ap_count : DEFAULT_SCAN_LIST_SIZE;
    memcpy(&scan_info_result.ap_info[0], &ap_info[0],
           sizeof(wifi_ap_record_t) * scan_info_result.ap_count);
  } else {
...
static void event_handler(void *arg, esp_event_base_t event_base,
  ...
  if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
    ...
    if (s_reconnect && ++s_retry_num < EXAMPLE_ESP_MAXIMUM_RETRY) {
      ...
      ESP_LOGE(TAG,
               "sta disconnected - max retries reached or reconnect disabled");
    }
...
static void wifi_reconnect_sta(void) {
  ...
  if (bits & WIFI_CONNECTED_BIT) {
    ...
    xEventGroupWaitBits(s_wifi_event_group, WIFI_FAIL_BIT, 0, 1,
                        portTICK_RATE_MS);
  }

  s_reconnect = true;
  s_retry_num = 0;
  ESP_LOGI(TAG, "wifi_reconnect_sta: SSID=[%s], Password=[%s]",
  ...
  xEventGroupWaitBits(s_wifi_event_group, WIFI_CONNECTED_BIT, 0, 1,
...
static void wifi_init_sta(void) {
  ...
  // Harden I2C stability: Lower WiFi TX power to reduce EMI/noise spikes
  // 17dBm = 68 * 0.25dBm. (Previously 14dBm/56 was too weak for some
  // environments)
  esp_wifi_set_max_tx_power(68);
  esp_wifi_set_ps(WIFI_PS_NONE);

  ESP_LOGI(TAG, "wifi_init_sta finished.%s, %s", wifi_config.sta.ssid,
...
static void network_task(void *args) {
  ...
  while (1) {
    if (pdPASS ==
        xQueueReceive(wifi_event_queue, &net_event, portTICK_RATE_MS / 5)) {
      switch (net_event) {
...
bool app_wifi_lock(uint32_t timeout_ms) {
  ...
  return xSemaphoreTakeRecursive(scan_info_result.wifi_mux, timeout_ticks) ==
         pdTRUE;
}
...
void app_network_start(void) {
  ...
  ESP_ERROR_CHECK_WITHOUT_ABORT((scan_info_result.wifi_mux) ? ESP_OK
                                                            : ESP_FAIL);

  wifi_event_queue = xQueueCreate(4, sizeof(net_event_t));
  ESP_ERROR_CHECK_WITHOUT_ABORT((wifi_event_queue) ? ESP_OK : ESP_FAIL);

  ret_val = xTaskCreatePinnedToCore(network_task, "NetWork Task", 5 * 1024,
                                    NULL, 1, NULL, 0);
