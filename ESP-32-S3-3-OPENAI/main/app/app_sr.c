    ...
    if (true == detect_flag) {
      ...
      if (ESP_MN_STATE_DETECTED == mn_state) {
        ...
        for (int i = 0; i < mn_res->num; i++) {
          ESP_LOGI(TAG, "ID: %d, Phrase ID: %d, Prob: %f",
...
esp_err_t app_sr_start(bool record_en) {
  ...
  g_sr_data = heap_caps_calloc(1, sizeof(sr_data_t),
                               MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
  ...
  model_iface_data_t *model_data =
      multinet->create(mn_name, 6000); // 6s timeout
  if (model_data == NULL) {
  ...
  if (ret != ESP_OK) {
    ...
    ESP_LOGI(TAG,
             "9 phrases registered successfully (including Check Sensors).");
  }
  ...
  ret_val =
      xTaskCreatePinnedToCore(&audio_feed_task, "Feed Task", 8 * 1024,
                              (void *)afe_data, 10, &g_sr_data->feed_task, 1);
  ESP_GOTO_ON_FALSE(pdPASS == ret_val, ESP_FAIL, err, TAG,
                    "Failed create audio feed task");

  ret_val =
      xTaskCreatePinnedToCore(&audio_detect_task, "Detect Task", 12 * 1024,
                              (void *)afe_data, 10, &g_sr_data->detect_task, 1);
  ESP_GOTO_ON_FALSE(pdPASS == ret_val, ESP_FAIL, err, TAG,
                    "Failed create audio detect task");

  xTaskCreatePinnedToCore(&sr_handler_task, "SR Handler Task", 8 * 1024, NULL,
                          5, &g_sr_data->handle_task, 1);
  ESP_GOTO_ON_FALSE(pdPASS == ret_val, ESP_FAIL, err, TAG,
...
esp_err_t app_sr_stop(void) {
  ...
  xEventGroupWaitBits(g_sr_data->event_group,
                      NEED_DELETE | FEED_DELETED | DETECT_DELETED |
                          HANDLE_DELETED,
                      1, 1, portMAX_DELAY);
